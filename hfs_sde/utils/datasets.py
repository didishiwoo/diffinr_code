...
class FastMRIKneeDataSet(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        with h5py.File(maps_file, "r") as data:
            ...
            maps = torch.from_numpy(np.asarray(maps_idx))
        ...
        with h5py.File(raw_file, "r") as data:
            ...
            if self.config.data.normalize_type == "minmax":
                ksp_tensor = torch.from_numpy(ksp_idx).unsqueeze(0)
                maps_tensor = maps.unsqueeze(0)
                img_idx = Emat_xyt_complex(ksp_tensor, True, maps_tensor, 1)
                img_idx = self.config.data.normalize_coeff * normalize_complex(img_idx)
                ksp_idx = Emat_xyt_complex(img_idx, False, maps_tensor, 1)
                kspace = ksp_idx.squeeze(0)
            elif self.config.data.normalize_type == "std":
                minv = np.std(ksp_idx)
                ksp_idx = ksp_idx / (self.config.data.normalize_coeff * minv)
                kspace = torch.from_numpy(np.asarray(ksp_idx))
